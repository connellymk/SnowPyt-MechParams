...
A lightweight extension of `Layer` that marks a specific snow layer as the weak layer beneath the slab. It inherits all `Layer` attributes (depth, thickness, density, hand hardness, grain form, grain size) so that the weak layer's physical properties can be read alongside the slab layers. No fracture or strength parameters are stored here — those are currently represented in the graph by the `weak_layer_info*` placeholder node, which signals that this information is not yet computed from pit observations.
...
In the parameter graph, the natural variant is exposed as a graph node: *s*_r (method `roch_natural`), reached via the merge node `merge_roch_inputs`, which aggregates the layer-level density parameter, layer thickness, and the `weak_layer_info*` placeholder. Because `weak_layer_info*` has no incoming method edges (weak-layer shear strength is not yet computed from pit observations), `find_parameterizations` returns **0 pathways** to *s*_r. The graph structure is preserved for future extension when site-specific τ_c measurements become available. The skier variant (S_a) is not exposed as a graph node; it is computed by calling `calculate_roch` directly with a caller-supplied δτ. The shear strength τ_c must be supplied by the caller in kPa; `calculate_roch` converts to Pa internally (τ_c_pa = τ_c × 1000) before computing the index. Results are written to `slab.roch_result`.
...
The graph connects the computational modules — `models`, `layer_parameters`, `slab_parameters`, and `stability_criteria` — by placing their inputs and outputs as nodes and their calculation methods as edges. This is the mechanism that allows the package to report 32 distinct pathways to D11: rather than maintaining a hand-written list of 32 method combinations, the algorithm traverses the graph and discovers them from its structure. When a new empirical method is added to the package, a single new edge in the graph is sufficient; the algorithm then finds every affected pathway automatically.

### Node types

The graph contains two kinds of node. A **parameter node** represents a named quantity — either a raw field measurement or a calculated result. The single root of the graph is a parameter node called `snow_pit`, representing the pit observation as a whole and having no incoming edges. Five measured-input parameter nodes flow from the root: `measured_density`, `measured_hand_hardness`, `measured_grain_form`, `measured_grain_size`, and `measured_layer_thickness`. Calculated outputs at the layer level (density, elastic modulus, Poisson's ratio, shear modulus), at the slab level (*A*11, *B*11, *D*11, *A*55), and the stability criterion outputs *g*_delta (WEAC skier criterion) and *s*_r (Roch natural stability index) are also parameter nodes, each tagged with a computation level. Additionally, a **`weak_layer_info*`** placeholder parameter node (level `"weak_layer"`) represents weak-layer fracture and strength data that are not yet computed from pit observations; it has no incoming method edges, so any target that requires it has zero computable pathways.

A **merge node** represents the convergence of two or more inputs into a single method call. Because most empirical methods require more than one input, a merge node is interposed between the parameter nodes supplying those inputs and the method edge that produces the result. For example, the four elastic modulus methods each require both density and grain form: the merge node `merge_density_grain_form` collects these two inputs and sits upstream of the four method edges that lead to the elastic modulus parameter node. At the slab level, separate merge nodes aggregate the layer-level results needed for each stiffness integral: `merge_E_nu` combines elastic modulus and Poisson's ratio from all layers, `merge_zi_E_nu` adds layer-position information required for the *D*11 bending integral, `merge_hi_G` combines thickness and shear modulus for the *A*55 shear integral, and `merge_hi_E_nu` combines thickness and the plane-strain modulus for the *A*11 and *B*11 integrals. A dedicated `slab_elasticity_parameters` merge node combines elastic modulus and Poisson's ratio (E + ν) and serves as the primary coverage analysis target for WEAC slab inputs — it has 32 computable pathways (4 density × 4 E-mod × 2 ν). The `merge_weac_inputs` node aggregates `slab_elasticity_parameters`, density, shear modulus, layer thickness, and `weak_layer_info*` before the WEAC criterion edge; the `merge_roch_inputs` node aggregates density, layer thickness, and `weak_layer_info*` before the Roch criterion edge. Because `weak_layer_info*` has no incoming method edges, both *g*_delta and *s*_r have zero computable pathways in the current graph.
...
The graph flows in a single direction from the root toward the target parameters. From `snow_pit`, data-flow edges fan out to all five measured-input parameter nodes. From there, the layer-level parameters are built up in stages: density can be reached via the direct-measurement data-flow edge (when a density cutter was used) or via one of three empirical methods that work from hand hardness, grain form, and optionally grain size; elastic modulus, Poisson's ratio, and shear modulus each follow from density and grain form through the `merge_density_grain_form` node, except that the kochle Poisson's ratio method branches from grain form alone. Once all layer-level parameters are available, the graph continues upward: the slab stiffness parameters are computed through the slab-level merge nodes, and `slab_elasticity_parameters` (E + ν) is assembled as a convenience target for stability coverage analysis. The stability criterion targets (*g*_delta and *s*_r) are wired through `merge_weac_inputs` and `merge_roch_inputs` respectively, but both depend on the `weak_layer_info*` placeholder node, which currently has no incoming method edges. Consequently, the algorithm finds **0 pathways** to either stability criterion output; the graph wiring is preserved for future extension when site-specific weak-layer data become available. The leaf nodes available as computable targets are the layer-level parameters, the slab stiffness parameters, and `slab_elasticity_parameters`.
...
In the second phase, the executor computes the requested slab-level parameter. *A*11, *B*11, *D*11, and *A*55 integrate layer-level properties across the full thickness of the slab, so they cannot be computed until all layer-level parameters are available on every layer. The two-phase order guarantees this prerequisite is met before the slab-level calculation is attempted. For stability criterion targets (*g*_delta, *s*_r), the graph currently returns 0 pathways because `weak_layer_info*` has no incoming method edges; the stability criteria can still be called directly by the user by supplying the required weak-layer parameters as function arguments. For the WEAC criterion, the result is stored in `slab.weac_result`; for the Roch natural criterion, in `slab.roch_result`.
...
The analysis notebook `stability_criteria_inputs.ipynb` characterises **slab-level input coverage** for the Roch and WEAC stability criteria across the full ECTP slab dataset.

**Roch section** (simplified coverage): reports what fraction of ECTP slabs have both density and layer thickness computable across the 4 density pathways. Because `weak_layer_info*` is a placeholder (no method edges), the Roch criterion is not executed; this section only identifies which slabs have the required slab-side inputs available.

**WEAC section** (slab elasticity coverage): reports what fraction of ECTP slabs have `slab_elasticity_parameters` (E + ν) computable across all 32 pathways (4 density × 4 E-mod × 2 ν). Coverage, per-parameter nominal values, and relative uncertainties are shown by method combination. Again the criterion itself is not executed; the target is the `slab_elasticity_parameters` merge node rather than *g*_delta.

The analysis is based on **14,951 ECTP slabs** drawn from 50,278 snow pits. Slabs were identified using the `ECTP_failure_layer` strategy — one slab per propagating ECTP result. All calculations use `include_method_uncertainty=False` so that the reported relative uncertainties reflect only the propagation of field measurement uncertainties, not regression model error.
