...
- **stability results** from the WEAC skier criterion, once calculated
...
The graph contains two kinds of node. A **parameter node** represents a named quantity — either a raw field measurement or a calculated result. The single root of the graph is a parameter node called `snow_pit`, representing the pit observation as a whole and having no incoming edges. Five measured-input parameter nodes flow from the root: `measured_density`, `measured_hand_hardness`, `measured_grain_form`, `measured_grain_size`, and `measured_layer_thickness`. All calculated outputs — density, elastic modulus, Poisson's ratio, and shear modulus at the layer level; *A*11, *B*11, *D*11, and *A*55 at the slab level; the six weak-layer fracture and strength parameters; and the stability criterion output *g*_delta — are also parameter nodes, each tagged with a computation level that tells the execution engine at which phase of the pipeline it can first be computed.
